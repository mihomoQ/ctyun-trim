...
Automated Restore is disabled in 0.1.3. A safe recovery engine must compare every current object with the post-Apply state and restore exact values without overwriting newer tasks, services or policy. Until that is implemented and tested, CTyunTrim fails closed.
...
There is no automated execution-guard, task, service, certificate, account or LocalGPO restore command in 0.1.3.
